...
const DownloadCsvAgGrid = ({ fileUploadResponse, setUpdates }) => {
  ...
  const addRow = () => {
    const existingProcessIDs = rowData
      .map((row) => row.Process_ID)
      .filter(Boolean);
    const existingRiskIDs = rowData.map((row) => row.Risk_ID).filter(Boolean);
    const existingControlIDs = rowData
      .map((row) => row.Control_ID)
      .filter(Boolean);
    const existingSubProcessIDs = rowData
      .map((row) => row.SubProcess_ID)
      .filter(Boolean);
  ...
  const onCellValueChanged = useCallback((params) => {
    setUpdates((prev) => {
      const existingIndex = prev.findIndex(
        (item) => item.Process_ID === params.data.Process_ID
      );
      if (existingIndex !== -1) {
        const updated = [...prev];
      ...
      {loading ? (
        ...
        <Paper sx={{ p: 2, pb: 7 }}>
          <div
            ...
            style={{
              width: "100%",
              height: rowData.length ? rowData.length * 40 + 90 : 250,
            }}
          >
            <StyledAg
              modules={[
